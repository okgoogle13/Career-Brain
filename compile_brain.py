...
==================================================
Reads all .txt files from normalized_vault/ and compiles them into three
LLM-optimised JSON engines using Pydantic validation:

  output/career_history.json      — Chronological roles + aggregated bullets
  output/ksc_and_narratives.json  — STAR/CAR/Pivot narratives indexed by competency
  output/skills_and_taxonomy.json — RBS-to-Community Rosetta Stone mapping
...
def extract_role_blocks(text: str) -> list[dict]:
    ...
    for i, line in enumerate(lines):
        ...
        if date_match:
            ...
            employer = detect_employer(context_text) or "Unknown"
            current_block = {
                "company": employer,
                "role": "Unknown",
